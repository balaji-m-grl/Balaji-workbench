...
- Q6.2 - Mapping philosophy
- Q6.3 - Mapping model (how one DSL rule links to USB-IF tests)
- Q6.4 - Concrete examples (SPR + EPR)
- Q6.5 - How this helps agents, validation, and certification workflows
- Q6.6 ## 1. Mapping philosophy (important first):
- Q6.7 * USB-IF test cases are:
- Q6.8 ** Procedural (step-by-step test descriptions)
- Q6.9 ** Observable-behavior based (what must be seen on the wire or VBUS)
- Q6.10 ** Often one test validates multiple spec clauses
- Q6.11 * Your DSL rules are:
- Q6.12 ** Declarative
- Q6.13 ** Spec-intent driven
- Q6.14 ** Composable and reusable
- Q6.15 * This is intentional and powerful. One DSL rule → multiple USB-IF test cases; one USB-IF test case → multiple DSL rules.
- Q6.16 ## 2. Mapping model (data structure):
- Q6.17 ### Extend each DSL rule with a certification mapping block:
- Q6.18 ```yaml
rule:
  id: EPR_CABLE_ELIGIBILITY
  scope: EPR_ENTRY
    ...
    usb_if:
      ...
      test_cases:
        ...
        - id: TD.PD.EPR.CABLE.3
          description: "Verify rejection of EPR with non-EPR cable"
```
- Q6.19 This block is non-executable but traceable.
- Q6.20 * Mapping patterns (you’ll reuse these a lot):
- Q6.21 ** Capability & policy rules
